...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1, 
        ...
        print('vis_level-->',vis_level) # 1
        downsample = self.level_downsamples[vis_level]
        print('downsample-->',downsample) # downsample--> (4.001205303334673, 4.001330376940133)
        scale = [1/downsample[0], 1/downsample[1]] # Scaling from 0 to desired level
        print('scale-->',scale) # scale--> [0.24992469123405964, 0.24991687908677826]

        if len(scores.shape) == 2:
            scores = scores.flatten()
        print('binarize--->',binarize) # False
        if binarize:
            if thresh < 0:
        ...
        if top_left is not None and bot_right is not None:
            ...
            w, h = region_size
            print('w:{:.4f},h:{:.4f}'.format(w,h)) # w=1742,h=1858   /  w=2489 h=2255
        
        print('before scale patch_size--->',patch_size) #(512,512)
        patch_size  = np.ceil(np.array(patch_size) * np.array(scale)).astype(int)
        coords = np.ceil(coords * np.array(scale)).astype(int)
        
        print('\ncreating heatmap for: ')
        print('top_left: ', top_left, 'bot_right: ', bot_right) #top_left:  (0, 0) bot_right:  (9959, 9023)
        print('w: {}, h: {}'.format(w, h)) #w=2489 h=2255
        print('scaled patch size: ', patch_size) # 128, 128

        ###### normalize filtered scores ######
        if convert_to_percentiles:
            scores = to_percentiles(scores) 
        print('to_percentiles_score--->',len(scores))#44
        scores /= 100
        ...
        print('overlay',overlay.size) # 2489x2255 = 5612695
        counter = np.full(np.flip(region_size), 0).astype(np.uint16)  
        print('counter',counter.size)    
        count = 0
        print('len(coords)',len(coords)) # 44
        ...
        for idx in range(len(coords)):
            ...
            #print('coord[0]+patch_size[0]',coord[0]+patch_size[0])
            #print('coord[1]',coord[1])
            #print('coord[1]+patch_size[1]',coord[1]+patch_size[1])
            if score >= threshold:
